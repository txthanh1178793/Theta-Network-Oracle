...
from core.utils import currency_pair_to_pair_id
from publisher.assets import Asset, SpotAsset
from publisher.types import PublisherFetchError, PublisherInterfaceT
...
class BitstampFetcher(PublisherInterfaceT):
    ...
    async def _fetch_pair(self, asset: SpotAsset, session: ClientSession) -> Union[SpotEntry, PublisherFetchError]:
        pair = asset["pair"]
    ...
    def _fetch_pair_sync(self, asset: SpotAsset) -> Union[SpotEntry, PublisherFetchError]:
        pair = asset["pair"]
        url = f"{self.BASE_URL}/{pair[0].lower()}{pair[1].lower()}"
    ...
    async def fetch(self, session: ClientSession) -> List[Union[SpotEntry, PublisherFetchError]]:
        entries = []
